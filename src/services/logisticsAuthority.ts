...
export type DeliveryProofVerifier = (params: {
  uid: string;
  attemptId: string;
  proofStoragePath: string;
}) => Promise<unknown>;
...
function normalizeUpperStatus(value: unknown) {
...
export async function assignPackageAuthority(params: {
  db: FirestoreLike;
  packageId: string;
  riderId: string;
  actorUid: string;
...
export async function transferAssignmentAuthority(params: {
  db: FirestoreLike;
  packageId: string;
...
export async function approveCodAllocationAuthority(params: {
  db: FirestoreLike;
  reviewId: string;
...
export async function recordDeliveryAttemptAuthority(params: {
  db: FirestoreLike;
  auth: DeliveryAttemptAuth;
  body: DeliveryAttemptRequest;
  verifyDeliveryProofStorageObject?: DeliveryProofVerifier;
}) {
  ...
  const result = await db.runTransaction(async (t: any) => {
    ...
    let codCollectionRecord: any = null;
